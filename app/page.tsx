...
export default async function Dashboard({ searchParams }: { searchParams: Promise<{ city?: string }> }) {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="flex justify-between items-start">
              <div className="space-y-2">
                <h1 className="text-5xl md:text-7xl lg:text-8xl font-black tracking-tighter">
                  {data.name} <span className="text-sky-200/30 text-3xl md:text-5xl">{data.sys.country}</span>
                </h1>
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
              <div className="flex flex-col">
                <div className="flex items-start">
                  <span className="text-9xl md:text-[12rem] font-black tracking-tighter leading-none">{Math.round(data.main.temp)}</span>
                  <span className="text-4xl md:text-6xl font-black mt-4">°</span>
                </div>
                <p className="text-2xl md:text-4xl font-black capitalize text-sky-100/90 ml-2">{data.weather[0].description}</p>
              </div>
              <img
        ...
        <div className="bg-[#161e2e]/50 border border-white/5 rounded-[2.5rem] p-8 backdrop-blur-xl flex flex-col justify-center">
          <div className="flex items-center gap-3 mb-10">
            ...
            <h3 className="text-sm font-black uppercase tracking-[0.2em] text-gray-400">Atmosphere</h3>
          </div>
          
          <div className="space-y-8">
            <DetailRow icon={<Thermometer size={22} className="text-orange-400" />} label="Feels Like" value={`${Math.round(data.main.feels_like)}°`} />
            <DetailRow icon={<Wind size={22} className="text-teal-400" />} label="Wind Speed" value={`${data.wind.speed} km/h`} />
            <DetailRow icon={<Droplets size={22} className="text-blue-400" />} label="Humidity" value={`${data.main.humidity}%`} />
            <DetailRow icon={<Gauge size={22} className="text-pink-400" />} label="Pressure" value={`${data.main.pressure} hPa`} />
            <DetailRow icon={<Eye size={22} className="text-purple-400" />} label="Visibility" value={`${data.visibility / 1000} km`} />
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="flex items-center justify-between group transition-all">
      <div className="flex items-center gap-4">
        ...
        <span className="text-gray-400 font-bold text-sm md:text-base">{label}</span>
      </div>
      <span className="text-xl md:text-2xl font-black text-gray-100 tracking-tight">{value}</span>
    </div>
  );
}