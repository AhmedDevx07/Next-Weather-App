...
export default function SearchInput() {
  ...
  return (
    <form onSubmit={handleSearch} className="relative w-full group">
      ...
      />
      <Search
        className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-sky-500 transition-colors"
        size={20}
      />
      <button
        type="submit"
        className="absolute right-3 top-1/2 -translate-y-1/2 bg-sky-500 hover:bg-sky-600 text-white px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all active:scale-90 shadow-lg shadow-sky-500/20"
      >
        Search
      </button>
    </form>
