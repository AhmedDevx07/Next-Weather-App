...
export default async function AirQualityPage({
  ...
  return (
    <div className="space-y-10 md:space-y-14 animate-in fade-in slide-in-from-bottom-6 duration-1000">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="space-y-2">
          ...
          <p className="text-gray-500 font-bold text-sm uppercase tracking-widest">
            Pollutant concentration in {weather.name}
          </p>
        </div>
      </div>
      ...
      {/* Main AQI Hero Card */}
      <div
        className={`relative overflow-hidden p-8 md:p-12 rounded-[3rem] border ${currentLevel.border} ${currentLevel.bg} backdrop-blur-xl shadow-2xl ${currentLevel.glow}`}
      >
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-10">
          <div className="text-center md:text-left space-y-4">
            ...
            <h3
              className={`text-6xl md:text-8xl font-black tracking-tighter ${currentLevel.color}`}
            >
              {currentLevel.label}
            </h3>
            <p className="text-gray-400 font-bold text-lg max-w-md italic">
              The air in {weather.name} is currently rated as{" "}
              {currentLevel.label.toLowerCase()}.
            </p>
          </div>
          ...
          <div className="relative flex items-center justify-center">
            {/* Visual Ring */}
            <div
              className={`h-48 w-48 md:h-64 md:w-64 rounded-full border-[16px] border-white/5 flex flex-col items-center justify-center relative shadow-inner`}
            >
              <span
                className={`text-6xl md:text-8xl font-black ${currentLevel.color} tracking-tighter`}
              >
                {aqi}
              </span>
              ...
              {/* Dynamic Glow Ring */}
              <div
                className={`absolute inset-[-16px] rounded-full border-[4px] ${currentLevel.border} opacity-50`}
              />
            </div>
          </div>
        </div>
  ...
  return (
    <div className="group relative bg-[#161e2e]/40 backdrop-blur-xl p-8 rounded-[2.5rem] border border-white/5 hover:border-sky-500/40 transition-all duration-500 overflow-hidden">
      ...
      <div className="relative z-10 flex justify-between items-start mb-6">
        <p className="font-black text-2xl text-white tracking-tighter group-hover:text-sky-400 transition-colors">
          {label}
        </p>
        <span className="text-[9px] bg-white/5 px-3 py-1 rounded-full border border-white/10 text-gray-500 uppercase font-black tracking-widest">
          {unit}
        </span>
      </div>
